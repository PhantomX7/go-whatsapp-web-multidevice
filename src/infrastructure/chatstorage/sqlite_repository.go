...
// GetOldestMessage returns the oldest stored message for a chat on a device.
// It is used as the anchor for on-demand history sync requests, which fetch
// messages immediately before a known message.
func (r *SQLiteRepository) GetOldestMessage(deviceID, chatJID string) (*domainChatStorage.Message, error) {
	if deviceID == "" {
...
func (r *SQLiteRepository) StoreMessage(message *domainChatStorage.Message) error {
	...
	result, err := r.db.Exec(`
		UPDATE messages SET sender = ?, content = ?, timestamp = ?, is_from_me = ?,
			media_type = ?, call_metadata = ?, filename = ?, url = ?, media_key = ?, file_sha256 = ?,
			file_enc_sha256 = ?, file_length = ?, referral_metadata = ?, direct_path = ?, updated_at = ?
		WHERE id = ? AND chat_jid = ? AND device_id = ?
	`, message.Sender, message.Content, message.Timestamp, message.IsFromMe,
		...
		message.FileEncSHA256, message.FileLength, message.ReferralMetadata, message.DirectPath, message.UpdatedAt,
	...
	if rowsAffected == 0 {
		...
		`, message.ID, message.ChatJID, message.DeviceID, message.Sender, message.Content,
			...
			message.FileLength, message.ReferralMetadata, message.DirectPath, message.CreatedAt, message.UpdatedAt)
	}
	return err
}
...
func (r *SQLiteRepository) CreateMessage(ctx context.Context, evt *events.Message) error {
	...
	message := &domainChatStorage.Message{
		...
		Filename:         filename,
		URL:              url,
		DirectPath:       utils.ExtractMediaDirectPath(evt.Message),
		MediaKey:         mediaKey,
...
func (r *SQLiteRepository) storeEditedMessage(ctx context.Context, evt *events.Message, deviceID, chatJID, sender string, editedMessage *waE2E.Message) error {
	...
	if currentMessage == nil {
		...
		if mediaType, filename, url, mediaKey, fileSHA256, fileEncSHA256, fileLength := utils.ExtractMediaInfo(editedMessage); mediaType != "" || newContent != "" {
			...
			currentMessage.DirectPath = utils.ExtractMediaDirectPath(editedMessage)
	...
	if messageExists {
		...
		`, currentMessage.ID, currentMessage.ChatJID, currentMessage.DeviceID, currentMessage.Sender, currentMessage.Content,
			...
			currentMessage.FileLength, currentMessage.ReferralMetadata, currentMessage.DirectPath, now, now); err != nil {
...
func (r *SQLiteRepository) StoreSentMessageWithContext(ctx context.Context, messageID string, senderJID string, recipientJID string, content string, timestamp time.Time, msg *waE2E.Message) error {
	...
	message := &domainChatStorage.Message{
		...
		Filename:      filename,
		URL:           mediaURL,
		DirectPath:    utils.ExtractMediaDirectPath(msg),
		MediaKey:      mediaKey,
		FileSHA256:    fileSHA256,
		FileEncSHA256: fileEncSHA256,
		FileLength:    fileLength,
...
func (r *SQLiteRepository) getMigrations() []string {
	return []string{
		...
		// Migration 23: Store media direct_path (required by whatsmeow to download media)
		`ALTER TABLE messages ADD COLUMN direct_path TEXT DEFAULT ''`,

		// Migration 24: Map WhatsApp messages to Chatwoot messages for idempotency and status sync
		...
		// Migration 29: Persist live Chatwoot forward retries across restarts
		...
		// Migration 30: Fetch due Chatwoot retry jobs in stable order
